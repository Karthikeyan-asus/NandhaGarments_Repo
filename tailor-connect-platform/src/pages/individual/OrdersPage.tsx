...
export default function IndividualOrdersPage() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      setIsLoading(true);
      try {
        // For individual users, fetch orders by user
        const response = await ordersService.getOrdersByUser(user.id, user.role);
        ...
        {orders.length === 0 ? (
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-16">
              ...
              <Button asChild>
                <a href="/individual/products">Browse Products</a>
              </Button>
            </CardContent>
          </Card>
        ) : (
